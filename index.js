...
module.exports = app => {
  ...
  app.on(['pull_request.opened', 'pull_request.synchronized', 'pull_request.reopened'], async context => {
    ...
    for (let i = 0; i < projects.length; i++) {
      if (projects[i].name === context.payload.repository.name) {
        project = projects[i]
      }
    }
    ...
    if (project !== null) {
      ...
      for (let i = 0; i < environments.length; i++) {
        if (environments[i].name === 'review/' + context.payload.pull_request.head.ref) {
          environment = environments[i]
        }
      }
      // If the environment exists mark it as deployed
      if (environment !== null && environment.external_url !== undefined) {
